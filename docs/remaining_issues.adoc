...
* `useLotAllocation` フックを `useLotAllocationLogic.ts`, `useLotAllocationActions.ts` 等に統合・再編
* `components/shared/LineBasedAllocationList` を `components/allocation-list/` に移動しコロケーションを改善

===== 10. `backups/` ディレクトリの運用方針 🔵

**問題**: `backups/` ディレクトリに手動バックアップが散在している。自動化や保存期間（リテンションポリシー）が未定義。

**対応**: ✅ 実装完了 (2025-11-28)

* 運用方針を `docs/operations/backup_policy.adoc` に策定
* 管理スクリプト `scripts/backup_manager.py` を実装（作成・自動削除）
...
* 入荷受入ロジックを `InboundReceivingService` に分離
...
* インポート処理を `ForecastImportService` に分離

===== 13. ForecastDetailCard の肥大化 🔵

**問題**: `frontend/src/features/forecasts/components/ForecastDetailCard/` に26ファイルが存在し、機能が集中している。現在はコロケーションされているため管理可能だが、今後さらに機能追加される場合は分割を検討すべき。

**対応**: ⏳ 監視継続（サブコンポーネントへの分割が望ましいが、規模が大きいため今回は見送り。将来的なIssueとして管理）
...
**Phase 3 完了状況**: ✅ 4/5 進行中

* ✅ allocations feature の複雑さ解消
* ✅ backups 運用方針
* ✅ InboundService 責務分離
* ✅ ForecastService 責務分離
* ✅ ForecastDetailCard 監視（将来対応として記録）
