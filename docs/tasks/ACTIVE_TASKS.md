...
現在の受注管理は2段階構成:
1. 受注一覧（Order List）
2. 受注詳細ページ（Order Detail）← クリックで遷移

**要望:**
受注一覧ページで**全ての明細行を直接フラット表示**し、詳細ページへのクリック遷移を不要にする。

**修正対象:**
- `frontend/src/features/orders/pages/OrdersListPage.tsx`
- `frontend/src/features/orders/components/OrdersFlatView.tsx`
- `frontend/src/features/orders/components/OrderLineColumns.tsx`

---

### P1-2: ロット新規登録 500エラー - ✅ 完了
...
### P1-3: データモデル再設計（商品マスタ整理）- ✅ 完了

**実施日:** 2025-12-10

- `product_mappings` テーブル作成
- `product_suppliers` へデータ自動生成（21件）

---

### P1-4: 納入先-得意先関連付け - 未調査
...
2. 親データ削除時に子データもカスケード削除（例: 得意先削除→関連データも削除）
3. 物理削除/論理削除の確認と統一

**修正対象:**
- 商品マスタ修正時に併せて対応
- `frontend/src/features/masters/*` 各ページ
