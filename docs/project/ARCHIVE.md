...
### 1-B. 納入先マスタ `次区コード` 必須化の最終仕上げ（後方互換解消） ✅ 対応済み
**完了**: 2026-02-07
**カテゴリ**: データ整合性・マスタ運用
**対応内容**:
- 既存 `delivery_places` の空/NULL `jiku_code` を `M{id}` でバックフィル
...
- 全完了条件を満たした
**ブランチ**: `codex/help-markdown-rendering` (#559)

---

## 2026-02-06 完了タスク

### 5-0. テスト用DBコンテナのネットワーク接続問題 ✅ 対応済み
**完了**: 2026-02-06
**カテゴリ**: テスト基盤・インフラ
**対応内容**:
- `backend` コンテナから `db-postgres-test` が名前解決できない問題を解決
- `docker-compose.yml` でサービス名を `db`, `db-test` に簡略化・統一
- `docker-compose.test.yml` を廃止し、メインの構成ファイルに統合
- `scripts/setup_test_db.py` を導入し、クロスプラットフォーム対応

---

## Excelビュー機能 完了タスク

### 10-1. 収容数・保証期間フィールドの実装 ✅ 対応済み
**状態**: 完了
**カテゴリ**: Excelビュー機能
**対応内容**:
- ヘッダーの「収容数」「保証期間」表示を実装
- `supplier_items` カラム追加 → `v_lot_receipt_stock` → Inventory API → Excel Viewヘッダー反映

### 10-2. 先方品番（customerPartNo）の表示 ✅ 対応済み
**状態**: 完了
**カテゴリ**: Excelビュー機能
**対応内容**:
- `customer_items` を `supplier_item_id` で取得し、得意先IDごとの先方品番をマッピング

### 10-3. 発注NO.の表示 ✅ 対応済み
**状態**: 完了
**カテゴリ**: Excelビュー機能
**対応内容**:
- `lot_receipts.order_no` を追加し、Excel Viewで表示/更新

### 10-5. 同一ロット番号・入荷日別の行表示確認 ✅ 対応済み
**状態**: 完了
**カテゴリ**: Excelビュー機能
**対応内容**:
...
### 10-7. 納入先追加の反映遅延 ✅ 対応済み
**状態**: 完了
**カテゴリ**: Excelビュー機能
**対応内容**:
- 納入先追加後の再取得を強化し、反映遅延を解消

### 10-8. コメント・手動出荷日の保存反映確認 ✅ 対応済み
**状態**: 完了
**カテゴリ**: Excelビュー機能
**対応内容**:
- 数量0でもコメント/手動出荷日が保存されるようにし、保存 → 再読み込みで反映確認

### 10-9. ロット重複時のエラーハンドリング ✅ 対応済み
**状態**: 完了
**カテゴリ**: Excelビュー機能
**対応内容**:
- 既存ロット番号保存時のトースト/バリデーション表示改善

### 10-10. 納入先行の並び替え機能 ✅ 対応済み
**状態**: 完了
**カテゴリ**: Excelビュー機能
**対応内容**:
...
### 1-1. SmartRead横持ちCSVフォーマット検証の確認 ✅ 対応済み
**完了**: 2026-02-05
**カテゴリ**: RPA・データ処理
**対応内容**:
- 横持ちCSVから縦持ちへの変換において、必要なカラムが存在すれば他のカラムの有無に関わらずエラーにならない仕様であることをコードベースで確認。
- カラム順序の柔軟性についても確認済み。

### 2-7. ライブラリのメジャーアップデート対応 ✅ 対応済み
**完了**: 2026-02-05
**カテゴリ**: メンテナンス・技術債
**対応内容**:
- `websockets` を 15.0.1 -> 16.0.0 にアップデート。ログストリーミング機能の正常動作を確認。
- `pandas` を 2.3.3 -> 3.0.0 にアップデート。
- `pandas` 3.0.0 での `str` 型定義厳格化による `iloc` 代入時の `TypeError` を修正。
- 全 567 件のバックエンドテストをパス。
**ブランチ**: `feature/upgrade-libraries-sumup`
