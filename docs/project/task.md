...
**根本原因**: Phase 2 スキーマ変更により、`customer_part_no` → `customer_part_no` にリネームされたが、テストコードが旧スキーマのまま。

**失敗テスト一覧** (9件):

| テスト名 | エラー種別 | 説明 |
|---------|-----------|------|
| `test_list_customer_items_with_filters` | TypeError | `customer_part_no` は無効なキーワード |
| `test_list_customer_items_by_customer` | TypeError | 同上 |
| `test_create_customer_item_success` | 422 != 201 | スキーマ不一致 |
| `test_create_customer_item_duplicate_returns_409` | TypeError | `customer_part_no` は無効なキーワード |
| `test_update_customer_item_success` | TypeError | 同上 |
| `test_update_customer_item_not_found` | 405 != 404 | ルーティング不一致 |
| `test_delete_customer_item_success` | TypeError | `customer_part_no` は無効なキーワード |
| `test_delete_customer_item_not_found` | 405 != 404 | ルーティング不一致 |
| `test_bulk_upsert_customer_items` | TypeError | `customer_part_no` は無効なキーワード |

**修正方針**:
1. `customer_part_no` → `customer_part_no` にリネーム
...
## 完了タスク
- [x] Phase 2-1: `supplier_items`, `customer_items` スキーマ拡張および基本ロジック実装 (2026-01-25)
- [x] `v2.1` 計画書のアーカイブ化 (2026-01-25)
