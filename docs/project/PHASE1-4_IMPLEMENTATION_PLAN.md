...
- [x] トースト通知が画面右下に表示される
- [x] `display_strategy="deferred"` の通知はトースト表示されない
- [x] `display_strategy="immediate"` の通知はトースト表示される
- [x] 通知センターに全通知が履歴として残る
- [x] 既存の通知機能が正常動作する（破壊的変更なし）
- [x] すべてのテストがパスする
- [x] 品質チェック（Lint, Format, Typecheck）がパスする
...
- [x] グローバルナビゲーションが3層構造で表示される
- [x] 「受注管理」メニューが `/ocr-results` にリンクされている
- [x] 「入荷予定」メニューが表示されている
- [x] 旧「受注管理」が非表示になっている（feature flag で制御可能）
- [x] RPA メニューが「業務自動化」セクションに表示される（アイコン: Zap⚡）
- [ ] OCR取込ページが `/ocr-results/import` で表示される（未実装）
- [ ] 手動取込機能が正常動作する（未実装）
- [ ] 自動取込設定が保存・読み込みできる（未実装）
- [ ] 自動取込スケジューラーが1分おきに動作する（未実装）
- [x] すべてのテストがパスする
- [x] 品質チェック（Lint, Format, Typecheck）がパスする

**実装完了日:** 2025-02-05（ナビゲーション再編のみ完了）
...
- [x] グローバルナビゲーションに「システム管理」セクションが表示される
- [ ] システム設定ページがタブ構造で表示される（将来タスク）
- [x] ユーザー管理ページが独立している（/admin/users → /settings/usersへリダイレクト）
- [x] ログビューアページが独立している（リアルタイム・クライアント・操作ログを統合）
- [x] 通知設定ページが表示される（プレースホルダー実装）
- [x] Admin ロール以外はシステム管理セクションが表示されない
- [x] すべてのテストがパスする
- [x] 品質チェック（Lint, Format, Typecheck）がパスする
...
### 調査フェーズ（完了）
- [x] 現状の排他制御実装を調査完了
- [x] 同時編集の可能性があるテーブルを特定
- [x] 推奨実装方式を決定（ハイブリッド方式）
- [x] 実装設計書を作成（PHASE4_CONCURRENCY_CONTROL_INVESTIGATION.md）

**調査完了日:** 2025-02-05

### 実装フェーズ（未着手）
- [ ] （P0）SmartReadデータに楽観的ロック実装（2-3日）
- [ ] （P1）セーブポイント導入（1-2日）
- [ ] （P1）ロック自動クリーンアップ（0.5日）
- [ ] （P2）OCR結果保護（詳細確認後）
- [ ] 編集中ユーザー表示機能を実装
- [ ] テストとドキュメント作成

**次のステップ:** 全体のテーブル構造を把握した上で、実装フェーズに進む
