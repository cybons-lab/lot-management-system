...
現在のアクションアイテム（未対応の課題）はすべて [BACKLOG.md](../BACKLOG.md) に集約されています。
...
- [ ] **ビジネス要件への適合**: 先方品番を主軸とした設計への見直し。
  - → [BACKLOG.md § 6-7: 商品識別設計のビジネス実態への適合](../BACKLOG.md#6-7-商品識別設計のビジネス実態への適合) へ移行。
...
- [x] **自動引当失敗時の挙動**: 現状はログ出力（WARNING）のみとし、将来的にリトライ機構等を検討する。
  - → 詳細は [BACKLOG.md § 1-3: 在庫計算ロジックの厳密化とSSOT固定](../BACKLOG.md#1-3-在庫計算ロジックの厳密化とssot固定) に関連。
...
- [ ] **計算ロジックのSSOT固定**: 利用可能在庫の計算式不備の解消。
  - → [BACKLOG.md § 1-3: 在庫計算ロジックの厳密化とSSOT固定](../BACKLOG.md#1-3-在庫計算ロジックの厳密化とssot固定) へ移行。
- [ ] **スケーラビリティ対応**: 100件上限の撤廃とサーバーサイドページング。
  - → [BACKLOG.md § 3-3: 大量データ表示の完全対応](../BACKLOG.md#3-3-大量データ表示の完全対応-ページネーション) へ移行。
- [ ] **DBトリガーによる自動同期**:
  - → [BACKLOG.md § 6-8: DB整合性維持の自動化](../BACKLOG.md#6-8-phase-4以降将来的な改善拡張) (Phase 4項目6) へ移行。
