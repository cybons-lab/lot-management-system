# ER図 - Lot Management System

データベーススキーマの全体的な構造と各テーブル間の関係を示します。
...
### 予測・提案 (3テーブル)
- **forecast_current**: 現在の予測
- **forecast_history**: 予測履歴
- **allocation_suggestions**: 引当サジェスト
...
### その他 (7テーブル)
- **product_uom_conversions**: 商品単位変換
- **allocation_traces**: 引当トレース
- **business_rules**: ビジネスルール
- **system_configs**: システム設定
- **batch_jobs**: バッチジョブ
- **seed_snapshots**: シードスナップショット
- **alembic_version**: Alembicバージョン管理

## 主要なリレーションシップ

### 受注フロー
```
customers → orders → order_lines → allocations → lots
```

### 入荷フロー
```
suppliers → inbound_plans → inbound_plan_lines → expected_lots → lots
```

### 予測フロー
```
customers → forecast_current → allocation_suggestions → lots
```

### マスタデータ関連
```
customers → customer_items ← products
customers → delivery_places
products → product_uom_conversions
```

## 関連ファイル
- スキーマダンプ: [schema_dump.sql](file:///Users/kazuya/dev/projects/lot-management-system/docs/schema_dump.sql)
- ビュー定義: プロジェクトルートの`apply_views_v2_5.sql`ディレクトリ
