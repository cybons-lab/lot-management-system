...
このプロジェクトでは、バックエンドのOpenAPI定義を **Single Source of Truth** とし、フロントエンドのTypeScript型定義を自動生成しています。

=== ワークフロー

バックエンドAPIを変更した場合、以下の手順で型定義を更新します：

[source,bash]
----
# 1. バックエンドで OpenAPI スキーマを再生成
cd backend
python scripts/export_openapi.py

# 2. フロントエンドで TypeScript 型定義を生成
cd ../frontend
npm run typegen
...
`package.json` に定義されているコマンド：

[source,json]
----
{
  "scripts": {
    "typegen": "openapi-typescript ../backend/openapi.json -o src/shared/types/openapi.d.ts && prettier --write src/shared/types/openapi.d.ts"
  }
}
----
...
. `openapi-typescript` で `openapi.json` から型定義を生成
. `prettier` で自動フォーマット
...
[source,typescript]
----
// src/shared/types/schema.ts でエイリアスを定義
import type { components } from './openapi';

export type CandidateLotItem = components['schemas']['CandidateLotItem'];
export type ManualAllocationSavePayload = components['schemas']['ManualAllocationSavePayload'];

// コンポーネントで使用
import type { CandidateLotItem } from '@/shared/types/schema';

function LotList({ lots }: { lots: CandidateLotItem[] }) {
  // ...
}
----
...
// ❌ Bad: openapi.d.ts を直接編集
// このファイルは自動生成されるため、変更は上書きされます
----
...
// ✅ Good: 必要に応じてエイリアスを schema.ts に追加
export type LotWithAllocation = CandidateLotItem & {
...
GitHub Actions の `schema-sync-check` ジョブは、コミットされた `openapi.d.ts` がバックエンドのスキーマと一致しているかを検証します。

失敗した場合：

. ローカルで `npm run typegen` を実行
. 生成された `openapi.d.ts` をコミット
. プッシュ
...
=== Styling Strategy（スタイル方針）

==== TailwindCSS は継続利用（ただし JSX には書かない）
...
=== Style Modules（*.styles.ts / styles.ts）の書き方

==== Tailwind はすべてスタイルファイルに逃がす
...
==== JSX からは className に直接 Tailwind を書かない
...
=== class-variance-authority (cva) の利用ルール

==== 状態・バリアントがある場合は cva を使う
...
. ✅ **ESLint Flat Config with strict rules** - Fully implemented
. ✅ **Feature-based directory structure** - All files reorganized
. ✅ **Split large files** - LotAllocationPage reduced from 941 to 169 lines
. ✅ **Eliminate "masters" naming** - Split into products, customers, suppliers, warehouses
. ✅ **Prettier with Tailwind plugin** - Configured and working
. ✅ **Build passes** - TypeScript compilation and Vite build successful
. ✅ **No behavior changes** - Only code organization, zero functionality changes
...
. ✅ **Feature directories** - 10 feature folders with proper structure
. ✅ **Shared infrastructure** - Consolidated in `shared/`
. ✅ **ESLINT_ISSUES.md** - Documented remaining issues
