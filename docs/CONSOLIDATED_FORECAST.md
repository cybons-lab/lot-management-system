...
1. [予測データ調査レポート](#1-予測データ調査レポート)
2. [予測データ問題サマリー](#2-予測データ問題サマリー)
3. [予測データ生成リファクタリング](#3-予測データ生成リファクタリング)
4. [予測生成Phase 1計画](#4-予測生成phase-1計画)
5. [シードデータジェネレータ比較](#5-シードデータジェネレータ比較)
...
2. 全製品に対して予測を生成
3. 日次・旬次・月次の3種類を作成
4. `forecast_quantity` はランダム（10～1000）
...
1. **CustomerItemベースで生成する（Phase 1）**
   - 存在しない商流で予測を作らない
   - データの整合性を保証
...
**確認手順:**
1. `forecasts=1` を指定したか確認
2. DBに `forecast_headers` / `forecast_lines` が存在するか確認
3. `forecast_period` が正しい形式（YYYY-MM）か確認
4. フロントエンドの `useForecastCalculations` が正しく動作しているか確認
