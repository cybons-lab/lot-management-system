...
- 倉庫（warehouses）: 4〜8 件  
- 製品（products）: 任意件数（数十件程度を想定）。製品コードは「PRD-123AB」、製品名は「Product-1234A」のような数字+アルファベット混在の体系とする。
- 顧客・製品マッピング: 各製品は原則として1社の主要顧客に紐付く。約15%の製品はレアケースとして2〜3社に共有される（より現実的な取引パターンを実現）

---

## 3. 在庫・ロット（lots）

**重要**: ロット生成はフォーキャストデータに基づいて計画的に行う。**1製品につき最大3ロットまで**とする。

各製品について、以下の条件でロットを生成する：

- **フォーキャストあり製品（70%）**:
  - **80%の製品**: 1ロット（フォーキャスト日別合計の±5%）→ **1ロットから割り当て**
  - **20%の製品**: 2ロット（1つはフォーキャスト日別合計、1つはランダム50〜150個）→ **2ロットから選択**
- **フォーキャストなし製品（25%）**: 1〜2ロット（100〜300個/ロット）
- **過剰仕入パターン（5%）**: 3〜5ロット（15〜40個/ロット）でテストケースとして組み込む
  - **ただし最大3ロットまでに制限**

- expiry（有効期限）: 今日+30〜120 日後の範囲でランダム  
...
- **旬（旬フォーキャスト）**  
  - 対象期間: 日別と同じく「翌月」  
  - 中旬 / 下旬 の 2 区分（SAP形式に合わせて上旬は含まない）
...
テスト目的で、50〜80 件の受注（全製品に分散）を生成する。

> 顧客と製品の取引関係（CustomerItem）に基づき、各顧客が取引している製品のみに対して受注を生成する。
