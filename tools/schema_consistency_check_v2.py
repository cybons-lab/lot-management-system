...
def identify_priorities(diff: Dict) -> Dict[str, List[str]]:
    ...
    for table, cols in diff["column_diffs"].items():
        for col in cols.get("model_only", []):
            if col in known_problematic:
                p0_issues.append(
                    f"`{table}.{col}` — Known legacy column, likely causes errors"
                )
            elif col.endswith("_code") and table not in [
                "products",
                "warehouses",
                "customers",
                "suppliers",
                ...
                p0_issues.append(
                    f"`{table}.{col}` — Model-only column (may cause errors)"
                )
    ...
    for table, cols in diff["column_diffs"].items():
        if cols.get("ddl_only"):
            p1_issues.append(
                f"`{table}` has {len(cols['ddl_only'])} DDL-only columns: {', '.join(sorted(cols['ddl_only']))}"
            )
...
def generate_report(
    diff: Dict, ddl_tables: Dict, model_tables: Dict, priorities: Dict
) -> str:
    ...
    if priorities["P0"]:
        ...
        report.append(
            "These columns are referenced in models but don't exist in DDL.\n"
        )
    ...
    if diff["tables_only_in_ddl"]:
        for table in diff["tables_only_in_ddl"]:
            ...
            report.append(
                f"- **`{table}`** — Columns: {', '.join(cols[:5])}{'...' if len(cols) > 5 else ''}"
            )
    else:
        report.append("*(None)*")
    report.append("")
    ...
    if diff["column_diffs"]:
        for table in sorted(diff["column_diffs"].keys()):
            ...
            if cols["model_only"]:
                report.append(
                    "**⚠️ Columns in Model only (NOT in DDL):** — **HIGH PRIORITY**"
                )
...
def main():
    ...
    print(
        f"  ✅ Found {len(priorities['P0'])} P0, {len(priorities['P1'])} P1, {len(priorities['P2'])} P2 issues"
    )
