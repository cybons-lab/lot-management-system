...
def parse_schema_sql(path: Path) -> list[ColumnDef]:
    """
    docs/schema/03_db_schema_current.sql から
    テーブルとカラム定義 + PK/FK を抜き出す。
    """
    text = path.read_text(encoding="utf-8")

    # CREATE TABLE ... (...) をテーブル単位で抜き出す
    ...
    for block in table_blocks[1:]:
        ...
        for line in lines:
            ...
            # 例: id bigint GENERATED BY DEFAULT AS IDENTITY,
            m_col = re.match(
                r'^"?(?P<col>\w+)"?\s+(?P<type>[^,]+?)(?:,)?$',
                line_stripped,
            )
...
def build_table_api_matrix(
    ...
    # True/False -> ○/× に変換
    df["has_api"] = df["has_api"].map(lambda v: "○" if v else "×")
    return df


def make_sheet_name(table_name: str) -> str:
    """Excel シート名用に安全な名前を作る（31文字制限対応）。"""
...
def add_excel_table(ws, style_name: str) -> None:
    """シート全体を Excel のテーブル化し、テーブル名はランダムサフィックスで一意にする"""
    ...
    base_name = base_name[:20]  # 少し短めにしておく

    # ランダムサフィックスを付けてほぼ確実に一意にする
...
def main() -> None:
    ...
    if OUTPUT_XLSX.exists():
        try:
            ...
            raise SystemExit(
                f"❌ Cannot delete existing file: {OUTPUT_XLSX}\n"
                f"Excel で開きっぱなしの可能性があります。閉じて再実行してください。"
            )
