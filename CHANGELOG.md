...
- **[P1-1] 受注管理画面の完全統合 (2段階→1段階へ)**
  ...
  - PR: #297
- **[P1-4] 単位換算ページに仕入先フィルタを追加**
  - 一覧ページに仕入先プルダウンによる絞り込み機能を追加
  - 新規登録ダイアログにも仕入先フィルタを追加（製品選択前に仕入先で絞り込み可能）
  - `FormLabel` を `FormField` 外で使用していたランタイムエラーを修正
  - PR: #299
- **[P1-4] 仕入先商品マスタ機能実装**
  - 新規登録・編集・削除機能
  - 一覧表示、インポート機能
...
- **[P1-2] 納入先マスタ `jiku_code` 非null化**
  - Alembicマイグレーションで `jiku_code` を `nullable=False` に変更
  - モデル定義を更新
...
- **[Backend Tests] 全25件のテスト失敗を解決（283 passed, 0 failed達成）**
  ...
  - 認証/セッション問題（12件）: 
    - `get_db`の二重定義問題を解決（`app.api.deps`と`app.core.database`両方をオーバーライド）
    - `auth_service`のJWT username解析修正
