"""Update v_ocr_results view to use maker_name as supplier_name

Revision ID: a1b2c3d4e5f6
Revises: 7469e861ca23
Create Date: 2026-01-26

仕入先名称をshipping_master_curatedのmaker_name（メーカー名）から取得するように変更
"""
...
COMMENT ON VIEW public.v_ocr_results IS 'OCR結果ビュー（SmartRead縦持ちデータ + 出荷用マスタJOIN + SAP突合ステータス）。仕入先名称はmaker_nameから取得';
"""

# 元のビュー定義（ダウングレード用 - supplier_name を直接使用）
OLD_VIEW = r"""
CREATE OR REPLACE VIEW public.v_ocr_results AS
SELECT
    ld.id,
...
def downgrade() -> None:
    """Restore original v_ocr_results view using supplier_name directly."""
    op.execute("DROP VIEW IF EXISTS public.v_ocr_results CASCADE")
    op.execute(OLD_VIEW)
