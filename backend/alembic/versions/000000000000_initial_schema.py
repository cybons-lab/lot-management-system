...
def upgrade() -> None:
    ...
    # 7. Delivery Places
    op.create_table('delivery_places',
        sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
    ...
    op.create_table('warehouses',
        ...
        sa.Column('created_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.Column('updated_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.CheckConstraint("warehouse_type IN ('internal', 'external', 'supplier')", name='chk_warehouse_type'),
        sa.PrimaryKeyConstraint('id'),
    ...
    op.create_table('inbound_plans',
        ...
        sa.Column('created_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.Column('updated_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.CheckConstraint("status IN ('planned', 'partially_received', 'received', 'cancelled')", name='chk_inbound_plans_status'),
    ...
    op.create_table('inbound_plan_lines',
        sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
        ...
        sa.Column('created_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.Column('updated_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.ForeignKeyConstraint(['inbound_plan_id'], ['inbound_plans.id'], ),
        sa.ForeignKeyConstraint(['product_id'], ['products.id'], ),
        sa.PrimaryKeyConstraint('id')
    )

    # 12. Expected Lots
    op.create_table('expected_lots',
        sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
        ...
        sa.Column('created_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.Column('updated_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.ForeignKeyConstraint(['inbound_plan_line_id'], ['inbound_plan_lines.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_table('lots',
        ...
        sa.Column('current_quantity', sa.Numeric(precision=15, scale=3), server_default='0', nullable=False),
        sa.Column('allocated_quantity', sa.Numeric(precision=15, scale=3), server_default='0', nullable=False),
        sa.Column('unit', sa.String(length=20), nullable=False),
        ...
        sa.Column('inspection_status', sa.String(length=20), server_default='not_required', nullable=False),
        ...
        sa.CheckConstraint('allocated_quantity <= current_quantity', name='chk_lots_allocation_limit'),
        sa.CheckConstraint('current_quantity >= 0', name='chk_lots_current_quantity'),
        sa.CheckConstraint("status IN ('active', 'depleted', 'expired', 'quarantine', 'locked')", name='chk_lots_status'),
        sa.CheckConstraint("inspection_status IN ('not_required', 'pending', 'passed', 'failed')", name='chk_lots_inspection_status'),
        sa.ForeignKeyConstraint(['expected_lot_id'], ['expected_lots.id'], ),
        sa.ForeignKeyConstraint(['product_id'], ['products.id'], ),
        sa.ForeignKeyConstraint(['supplier_id'], ['suppliers.id'], ),
    ...
    op.create_table('order_lines',
        ...
        sa.CheckConstraint("status IN ('pending', 'allocated', 'shipped', 'completed', 'cancelled')", name='chk_order_lines_status'),
        sa.ForeignKeyConstraint(['delivery_place_id'], ['delivery_places.id'], ),
    ...
    op.create_table('allocations',
        ...
        sa.Column('created_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.Column('updated_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.CheckConstraint("status IN ('allocated', 'shipped', 'cancelled')", name='chk_allocations_status'),
    ...
    # 17. Forecast Current
    op.create_table('forecast_current',
        sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
        sa.Column('customer_id', sa.BigInteger(), nullable=False),
        sa.Column('delivery_place_id', sa.BigInteger(), nullable=False),
        sa.Column('product_id', sa.BigInteger(), nullable=False),
    ...
    op.create_table('allocation_suggestions',
        sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
        ...
        sa.Column('created_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.Column('updated_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.ForeignKeyConstraint(['forecast_line_id'], ['forecast_current.id'], ),
        sa.ForeignKeyConstraint(['lot_id'], ['lots.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_table('adjustments',
        ...
        sa.Column('adjusted_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.CheckConstraint("adjustment_type IN ('physical_count', 'damage', 'loss', 'found', 'other')", name='chk_adjustments_type'),
    ...
    op.create_table('stock_history',
        ...
        sa.CheckConstraint("transaction_type IN ('inbound', 'allocation', 'shipment', 'adjustment', 'return')", name='chk_stock_history_type'),
        sa.ForeignKeyConstraint(['lot_id'], ['lots.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_table('batch_jobs',
        ...
        sa.Column('created_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.CheckConstraint("status IN ('pending', 'running', 'completed', 'failed')", name='chk_batch_jobs_status'),
        sa.CheckConstraint("job_type IN ('allocation_suggestion', 'allocation_finalize', 'inventory_sync', 'data_import', 'report_generation')", name='chk_batch_jobs_type'),
        sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_table('business_rules',
        ...
        sa.Column('created_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.Column('updated_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.CheckConstraint("rule_type IN ('allocation', 'expiry_warning', 'kanban', 'other')", name='chk_business_rules_type'),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('rule_code')
    )

    # 25. Master Change Logs
    op.create_table('master_change_logs',
        sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
        ...
        sa.CheckConstraint("change_type IN ('insert', 'update', 'delete')", name='chk_master_change_logs_type'),
        sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_table('operation_logs',
        ...
        sa.Column('created_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.CheckConstraint("operation_type IN ('create', 'update', 'delete', 'login', 'logout', 'export')", name='chk_operation_logs_type'),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
