...
with engine.connect() as conn:
    ...
    print("-" * 80)
    for row in result:
        print(f"次区: {row.jiku_code or '(NULL)':15s} | 納入先コード: {row.delivery_place_code:20s} | {row.delivery_place_name}")
    ...
    # 重複チェック
    duplicates = conn.execute(text("""        SELECT jiku_code, COUNT(*) as cnt
        FROM delivery_places
