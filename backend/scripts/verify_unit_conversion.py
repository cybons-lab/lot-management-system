    ...
    try:
        ...
        try:
            ...
            try:
                db.execute(
                    text(
                        "ALTER TABLE orders ADD COLUMN status VARCHAR(20) DEFAULT 'open' NOT NULL;"
                    )
                )
            except Exception:
        ...
        delivery_place = (
            db.query(DeliveryPlace).filter(DeliveryPlace.customer_id == customer.id).first()
        )
        ...
        warehouse = db.query(Warehouse).first() or Warehouse(
            warehouse_code="WH1", warehouse_name="WH1", warehouse_type="internal"
        )
        if not warehouse.id:
            db.add(warehouse)
            db.commit()
