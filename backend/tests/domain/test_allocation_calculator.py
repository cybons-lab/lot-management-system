...
def lot_candidate_strategy(draw, lot_id=None):
    ...
    lot_number = draw(
        st.text(min_size=5, max_size=20, alphabet=st.characters(whitelist_categories=("Lu", "Nd")))
    )
...
class TestAllocationCalculatorProperties:
    ...
    def test_fefo_order_respected(self, request, candidates):
        ...
        for i in range(len(allocated_with_expiry) - 1):
            current_lot = next(c for c in candidates if c.lot_id == allocated_with_expiry[i].lot_id)
            next_lot = next(
