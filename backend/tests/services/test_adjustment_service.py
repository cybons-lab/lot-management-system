...
def test_create_adjustment_increase(db: Session, service_master_data):
    ...
    lm = LotMaster(supplier_item_id=product.id, supplier_id=supplier.id, lot_number="LOT-ADJ-1")
    db.add(lm)
    db.flush()
...
def test_create_adjustment_decrease(db: Session, service_master_data):
    ...
    lm = LotMaster(supplier_item_id=product.id, supplier_id=supplier.id, lot_number="LOT-ADJ-2")
    db.add(lm)
    db.flush()
...
def test_create_adjustment_deplete(db: Session, service_master_data):
    ...
    lm = LotMaster(supplier_item_id=product.id, supplier_id=supplier.id, lot_number="LOT-ADJ-3")
    db.add(lm)
    db.flush()
...
def test_create_adjustment_negative_balance_error(db: Session, service_master_data):
    ...
    lm = LotMaster(supplier_item_id=product.id, supplier_id=supplier.id, lot_number="LOT-ADJ-4")
    db.add(lm)
    db.flush()
...
def test_create_adjustment_lot_not_found(db: Session):
    ...
    adj_data = AdjustmentCreate(
        lot_id=99999, adjustment_type="found", adjusted_quantity=10, reason="Found", adjusted_by=1
    )
...
def test_get_adjustments_filtering(db: Session, service_master_data):
    ...
    lm = LotMaster(supplier_item_id=product.id, supplier_id=supplier.id, lot_number="LOT-ADJ-FILT")
    db.add(lm)
    db.flush()
...
def test_get_adjustment_by_id(db: Session, service_master_data):
    ...
    lm = LotMaster(supplier_item_id=product.id, supplier_id=supplier.id, lot_number="LOT-ADJ-GET")
    db.add(lm)
    db.flush()
