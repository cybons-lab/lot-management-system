...
def test_receive_inbound_plan_with_expected_lots(db: Session, service_master_data):
    ...
    plan = InboundPlan(plan_number="IP-RCV-001", supplier_id=supplier.id, planned_arrival_date=date.today(), status="planned")
    db.add(plan)
    db.flush()
    
    line = InboundPlanLine(inbound_plan_id=plan.id, product_id=product.id, planned_quantity=100, unit="EA")
    db.add(line)
    db.flush()
...
def test_receive_inbound_plan_without_expected_lots(db: Session, service_master_data):
    ...
    plan = InboundPlan(plan_number="IP-RCV-AUTO", supplier_id=supplier.id, planned_arrival_date=date.today(), status="planned")
    db.add(plan)
    db.flush()
    
    line = InboundPlanLine(inbound_plan_id=plan.id, product_id=product.id, planned_quantity=50, unit="EA")
    db.add(line)
    db.commit()
...
def test_receive_inbound_plan_already_received(db: Session, service_master_data):
    ...
    plan = InboundPlan(plan_number="IP-RCV-DONE", supplier_id=supplier.id, planned_arrival_date=date.today(), status="received")
    db.add(plan)
    db.commit()
...
def test_receive_inbound_plan_cancelled(db: Session, service_master_data):
    ...
    plan = InboundPlan(plan_number="IP-RCV-CNCL", supplier_id=supplier.id, planned_arrival_date=date.today(), status="cancelled")
    db.add(plan)
    db.commit()
