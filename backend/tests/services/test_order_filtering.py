...
from app.models import Order, OrderLine, Product, Customer, DeliveryPlace
...
def order_filtering_data(db: Session):
    ...
    product = Product(maker_part_code="PROD-FILTER", product_name="Filter Product", internal_unit="EA", base_unit="EA")
    db.add(product)
    
    delivery_place = DeliveryPlace(delivery_place_code="DP-FILTER", delivery_place_name="Filter Place", customer_id=1) # dummy fk
    ...
    db.flush()
    db.add(OrderLine(
        order_id=order1.id, product_id=product.id, delivery_place_id=delivery_place.id,
        order_quantity=Decimal("10"), unit="EA", delivery_date=date(2025, 1, 10),
        order_type="SPOT",
        status="pending"
    ))
    ...
    db.flush()
    db.add(OrderLine(
        order_id=order2.id, product_id=product.id, delivery_place_id=delivery_place.id,
        order_quantity=Decimal("20"), unit="EA", delivery_date=date(2025, 1, 11),
