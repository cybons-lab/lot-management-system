...
def test_create_inbound_plan_success(db: Session, service_master_data):
    ...
    assert plan.lines[0].supplier_item_id == product.id
...
def test_get_inbound_plans_filtering(db: Session, service_master_data):
    ...
    line1 = InboundPlanLine(
        inbound_plan_id=plan1.id, supplier_item_id=product1.id, planned_quantity=10, unit="EA"
    )
    db.add(line1)
    ...
    line2 = InboundPlanLine(
        inbound_plan_id=plan2.id, supplier_item_id=product2.id, planned_quantity=20, unit="KG"
    )
    db.add(line2)
    db.commit()
    ...
    # Test product filter
    plans, total = service.get_inbound_plans(supplier_item_id=product1.id)
    assert any(p.plan_number == "IP-FILT-1" for p in plans)
    assert not any(p.plan_number == "IP-FILT-2" for p in plans)
...
def test_delete_inbound_plan(db: Session, service_master_data):
    ...
    line = InboundPlanLine(
        inbound_plan_id=plan.id, supplier_item_id=product.id, planned_quantity=10, unit="EA"
    )
    db.add(line)
    db.commit()
...
def test_create_line(db: Session, service_master_data):
    ...
    assert line.supplier_item_id == product.id
...
def test_create_line_plan_not_found(db: Session, service_master_data):
    ...
    line_data = InboundPlanLineCreate(supplier_item_id=product.id, planned_quantity=50, unit="EA")
