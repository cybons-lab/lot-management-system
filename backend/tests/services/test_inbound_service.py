...
from app.models.inbound_models import InboundPlan, InboundPlanLine, ExpectedLot
...
def test_get_inbound_plans_filtering(db: Session, service_master_data):
    ...
    line1 = InboundPlanLine(inbound_plan_id=plan1.id, product_id=product1.id, planned_quantity=10, unit="EA")
    db.add(line1)
    ...
    line2 = InboundPlanLine(inbound_plan_id=plan2.id, product_id=product2.id, planned_quantity=20, unit="KG")
    db.add(line2)
    db.commit()
...
def test_get_inbound_plan_by_id(db: Session, service_master_data):
    ...
    plan = InboundPlan(plan_number="IP-GET-1", supplier_id=supplier.id, planned_arrival_date=date.today())
    db.add(plan)
    db.commit()
...
def test_update_inbound_plan(db: Session, service_master_data):
    ...
    plan = InboundPlan(plan_number="IP-UPD-1", supplier_id=supplier.id, planned_arrival_date=date.today(), status="planned")
    db.add(plan)
    db.commit()
...
def test_delete_inbound_plan(db: Session, service_master_data):
    ...
    plan = InboundPlan(plan_number="IP-DEL-1", supplier_id=supplier.id, planned_arrival_date=date.today())
    db.add(plan)
    db.flush()
    
    line = InboundPlanLine(inbound_plan_id=plan.id, product_id=product.id, planned_quantity=10, unit="EA")
    db.add(line)
    db.commit()
...
def test_create_line(db: Session, service_master_data):
    ...
    plan = InboundPlan(plan_number="IP-LINE-1", supplier_id=supplier.id, planned_arrival_date=date.today())
    db.add(plan)
    db.commit()
