"""Tests for order auto allocation (Single Lot Fit strategy).
...
def test_order_allocation_single_lot_fit(db: Session):
    ...
    # 4. Setup Order (order_number removed - using customer_order_no at line level)
    order = Order(
        order_date=today,
        status="open",
        customer_id=customer.id,
    ...
    order_line = OrderLine(
        ...
        status="pending",
        order_type="ORDER",
        customer_order_no="AUTO01",  # Business key at line level
    )
    db.add(order_line)
    db.commit()

    # 5. Execute Preview (New Logic)
