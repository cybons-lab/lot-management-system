...
def preview_allocation_suggestions(
    ...
    if request.mode == "forecast":
        if not request.forecast_scope or not request.forecast_scope.forecast_periods:
            raise HTTPException(status_code=400, detail="Forecast periods required for forecast mode")
...
def list_allocation_suggestions(
    ...
    total = query.count()
    suggestions = query.order_by(AllocationSuggestion.created_at.desc()).offset(skip).limit(limit).all()
