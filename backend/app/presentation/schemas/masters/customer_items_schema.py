...
class CustomerItemResponse(BaseSchema):
    ...
    shipping_document_template: str | None = Field(None, description="出荷表テンプレート")
    sap_notes: str | None = Field(None, description="SAP備考")
    maker_part_no: str | None = Field(None, description="メーカー品番")
    order_category: str | None = Field(None, description="発注区分")
    is_procurement_required: bool = Field(..., description="発注の有無")
    shipping_slip_text: str | None = Field(None, description="出荷票テキスト")
    ocr_conversion_notes: str | None = Field(None, description="OCR変換用備考")
    sap_supplier_code: str | None = Field(None, description="SAP仕入先コード")
    sap_warehouse_code: str | None = Field(None, description="SAP倉庫コード")
    sap_shipping_warehouse: str | None = Field(None, description="SAP出荷倉庫")
    sap_uom: str | None = Field(None, description="SAP単位")
