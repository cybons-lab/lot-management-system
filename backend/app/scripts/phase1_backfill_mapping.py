...
Strategy:
    customer_items の product_id を使って、対応する supplier_items を検索し、
    supplier_item_id を自動設定します。

    Matching logic:
    1. product_id が一致する supplier_items を検索
...
def backfill_supplier_item_mappings(session: Session, dry_run: bool = True) -> tuple[int, int, int]:
    """Backfill customer_items.supplier_item_id based on product_id.
    ...
    for item in unmapped:
        ...
        if product_id is None:
            print(f"  ⚠️  ID {customer_item_id}: No product_id, cannot map")
            skipped_count += 1
            continue

        # Find supplier_item for this product_id
        ...
        if not supplier_item:
            print(
                ...
                f"No supplier_item found for product_id={product_id}"
            )
            not_found_count += 1
            continue
