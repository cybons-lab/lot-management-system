...
class OcrImportService:
    ...
    def _process_line(
        ...
        """明細行を処理."""
        # マスタ検索で製品IDを解決
        supplier_item_id, match_type, message = self.complement_service.resolve_supplier_item_id(
        ...
        order_line = OrderLine(
            order_id=order_id,
            supplier_item_id=result.supplier_item_id,  # NULLの場合もあり
            customer_part_no=line.customer_part_no,
        ...
        # 結果を返す
        status = "resolved" if result.supplier_item_id else "unresolved"
