...
9. なぜ製品IDの一致を検証するのか（L80-83）
   ...
   → タイヤをブレーキパッドに引当（誤り）
   解決:
   - lot.supplier_item_id != line.supplier_item_id → ValueError
...
def create_manual_reservation(
    ...
    *,
    ...
    if lot.supplier_item_id != line.supplier_item_id:
        raise ValueError(
            f"Product mismatch: Lot product {lot.supplier_item_id} != Line product {line.supplier_item_id}"
        )
