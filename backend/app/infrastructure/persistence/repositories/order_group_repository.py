...
class OrderGroupRepository:
    ...
    def find_by_business_key(
        ...
        stmt = select(OrderGroup).where(
            ...
            OrderGroup.supplier_item_id == supplier_item_id,
    ...
    def upsert_by_business_key(
        ...
        if group_id is None:
            # 既存レコードを取得
            existing = self.find_by_business_key(customer_id, supplier_item_id, order_date)
            if existing is None:
                raise RuntimeError(
                    f"Failed to upsert OrderGroup: {customer_id}/{supplier_item_id}/{order_date}"
                )
            return existing
    ...
    def find_all(
        ...
        if supplier_item_id:
            stmt = stmt.where(OrderGroup.supplier_item_id == supplier_item_id)
        if date_from:
