...
test.describe("Product Mappings", () => {
  ...
  const setupMocks = async (page: import("@playwright/test").Page) => {
    ...
    await page.route("**/masters/customers*", async (route) =>
      route.fulfill({ json: [mockCustomer] }),
    );
    await page.route("**/masters/suppliers*", async (route) =>
      route.fulfill({ json: [mockSupplier] }),
    );
    await page.route("**/masters/products*", async (route) =>
      route.fulfill({ json: [mockProduct] }),
    );
  ...
  test("should display product mapping list with resolved names", async ({ page }) => {
    ...
    await expect(
      page.getByText(`${mockCustomer.customer_code} - ${mockCustomer.customer_name}`),
    ).toBeVisible();
    await expect(
      page.getByText(`${mockSupplier.supplier_code} - ${mockSupplier.supplier_name}`),
    ).toBeVisible();
    await expect(
      page.getByText(`${mockProduct.maker_part_code} - ${mockProduct.product_name}`),
    ).toBeVisible();
  });
