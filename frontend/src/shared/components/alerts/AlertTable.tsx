...
function getAlertTargetLabel(alert: AlertItem) {
  const { target } = alert;
...
function navigateByAlertTarget(navigate: ReturnType<typeof useNavigate>, alert: AlertItem) {
  const { target } = alert;
  switch (target.resource_type) {
    case "order":
...
const ALERT_TABLE_COLUMNS: Column<AlertItem>[] = [
  {
    id: "severity",
    header: "重要度",
...
export function AlertTable({ alerts, isLoading = false, onAlertClick }: AlertTableProps) {
  ...
  const handleAlertClick = (alert: AlertItem) => {
    ...
    navigateByAlertTarget(navigate, alert);
  };
