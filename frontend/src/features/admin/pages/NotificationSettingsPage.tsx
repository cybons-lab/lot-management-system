/**
 * NotificationSettingsPage.tsx
 *
 * 通知設定ページ（システム管理メニューから独立）
 * Phase 3: システム管理メニュー分割
 *
 * TODO: 通知ルール、メール通知設定、Slack連携などの機能を実装
 */

import { Bell, Mail, MessageSquare, AlertCircle } from "lucide-react";

import { PageContainer, PageHeader } from "@/shared/components/layout";

export function NotificationSettingsPage() {
  return (
    <PageContainer>
      <PageHeader title="通知設定" subtitle="通知ルールとチャンネル設定を管理します" />

      <div className="space-y-6">
        {/* 開発中の表示 */}
        <div className="bg-yellow-50 rounded-lg border border-yellow-200 p-6">
          <div className="flex items-start gap-3">
            <AlertCircle className="h-5 w-5 text-yellow-600 mt-0.5" />
            <div>
              <h3 className="text-lg font-semibold text-yellow-900 mb-2">開発中の機能</h3>
              <p className="text-sm text-yellow-800">
                この機能は現在開発中です。Phase 3では基本的な構造のみを実装しています。
              </p>
            </div>
          </div>
        </div>

        {/* 通知ルール（プレースホルダー） */}
        <div className="bg-card rounded-lg border p-6">
          <div className="flex items-center gap-2 mb-4">
            <Bell className="h-5 w-5 text-blue-500" />
            <h3 className="text-lg font-semibold">通知ルール</h3>
          </div>
          <p className="text-sm text-muted-foreground">
            イベント発生時の通知条件やルールを設定できます。
          </p>
          <div className="mt-4 text-xs text-muted-foreground italic">
            ※ 今後のアップデートで実装予定
          </div>
        </div>

        {/* メール通知設定（プレースホルダー） */}
        <div className="bg-card rounded-lg border p-6">
          <div className="flex items-center gap-2 mb-4">
            <Mail className="h-5 w-5 text-green-500" />
            <h3 className="text-lg font-semibold">メール通知設定</h3>
          </div>
          <p className="text-sm text-muted-foreground">
            メール送信設定（SMTPサーバー、送信元アドレスなど）を管理します。
          </p>
          <div className="mt-4 text-xs text-muted-foreground italic">
            ※ 今後のアップデートで実装予定
          </div>
        </div>

        {/* Slack連携（プレースホルダー） */}
        <div className="bg-card rounded-lg border p-6">
          <div className="flex items-center gap-2 mb-4">
            <MessageSquare className="h-5 w-5 text-purple-500" />
            <h3 className="text-lg font-semibold">Slack連携</h3>
          </div>
          <p className="text-sm text-muted-foreground">
            Slackワークスペースとの連携設定を行います。
          </p>
          <div className="mt-4 text-xs text-muted-foreground italic">
            ※ 今後のアップデートで実装予定
          </div>
        </div>
      </div>
    </PageContainer>
  );
}
