  ...
  return (
    <>
      ...
      <SoftDeleteDialog
        ...
        onOpenChange={(o) => !o && dlgs.close()}
        title="仕入先を無効化しますか？"
        description={`${dlgs.deletingItem?.supplier_name}（${dlgs.deletingItem?.supplier_code}）を無効化します。`}
      ...
      <PermanentDeleteDialog
        ...
        title="仕入先を完全に削除しますか？"
        description={`${dlgs.deletingItem?.supplier_name} を完全に削除します。`}
        confirmationPhrase={dlgs.deletingItem?.supplier_code || "delete"}
      />
      <RestoreDialog
        ...
        title="仕入先を復元しますか？"
        description={`${dlgs.restoringItem?.supplier_name} を有効状態に戻します。`}
      />
      <SupplierBulkDialog
        isAdmin={p.isAdmin}
        isOpen={p.isBulkOpen}
        ...
        onConfirmP={() => p.executeBulkWithStatus(p.permDel.mutateAsync, "完全削除")}
        onConfirmS={(e) =>
