...
const getItemKey = (item: InventoryItem) =>
  `${item.supplier_id ?? "all"}-${item.supplier_item_id}-${item.warehouse_id}`;
...
export function InventoryTable({
  ...
  const expandedRowIds = useMemo(
    () =>
      data
        .filter((item) => isRowExpanded(item.supplier_item_id, item.warehouse_id))
        .map(getItemKey),
    [data, isRowExpanded],
  );
  ...
  const handleExpandedRowsChange = (ids: (string | number)[]) => {
    ...
    [...idsSet]
      ...
      .forEach((added) => {
        ...
        if (item) void fetchLotsForItem(item.supplier_item_id, item.warehouse_id);
      });
    const expandKeys = new Set<string>();
    ids.forEach((id) => {
      const item = data.find((i) => getItemKey(i) === String(id));
      if (item) expandKeys.add(`${item.supplier_item_id}-${item.warehouse_id}`);
    });
  ...
  return (
    <div>
      ...
      <QuickLotIntakeDialog
        ...
        onOpenChange={(open) => !open && closeDialog()}
        onSuccess={handleSuccess}
        initialProductId={quickIntakeItem?.supplier_item_id}
