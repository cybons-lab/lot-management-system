...
const INVENTORY_BY_SUPPLIER_COLUMNS: Column<InventoryBySupplierResponse>[] = [
  {
    id: "supplier_code",
    header: "仕入先コード",
...
export function InventoryBySupplierTable({
  ...
  const renderRowActions = onViewDetail
    ? (row: InventoryBySupplierResponse) => (
        <SupplierDetailAction onViewDetail={onViewDetail} row={row} />
      )
    : undefined;
