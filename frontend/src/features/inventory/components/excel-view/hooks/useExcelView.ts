...
import { type NewDestinationFormData } from "../subcomponents/AddDestinationDialog";
...
export function useExcelView(productId: number, customerItemId?: number) {
  ...
  // -- Data Fetching --
  const { data, isLoading, supplierId, customerItem } = useExcelViewData(productId, customerItemId);
  ...
  return {
    ...
    handleArchiveLot: (lotId: number, lotNumber?: string) =>
