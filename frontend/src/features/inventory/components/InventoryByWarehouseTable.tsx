...
const INVENTORY_BY_WAREHOUSE_COLUMNS: Column<InventoryByWarehouseResponse>[] = [
  {
    id: "warehouse_code",
    header: "倉庫コード",
...
function InventoryDetailAction({
  onViewDetail,
  row,
}: {
  onViewDetail: (warehouseId: number) => void;
...
export function InventoryByWarehouseTable({
  ...
  const renderRowActions = onViewDetail
    ? (row: InventoryByWarehouseResponse) => (
        <InventoryDetailAction onViewDetail={onViewDetail} row={row} />
      )
    : undefined;
