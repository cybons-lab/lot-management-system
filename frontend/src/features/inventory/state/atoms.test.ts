...
describe("inventory lot derived logic", () => {
  ...
  it("sorts lots with the same behavior as existing table settings", () => {
    const lots = [createLot({ id: 1, product_id: 1 }), createLot({ id: 2, product_id: 2 })];
    const tableSettings: LotTableSettings = { sortColumn: "product_id", sortDirection: "desc" };

    const result = sortLots(lots, tableSettings);

    expect(result.map((lot) => lot.product_id)).toEqual([2, 1]);
  });
