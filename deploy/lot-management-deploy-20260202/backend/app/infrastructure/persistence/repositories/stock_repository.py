...
3. なぜ supplier_item_id と warehouse_id を使うのか（L33-35, L44-45）
   理由: DDL v2.2 準拠
   設計:
   - v1.x: product_code, warehouse_code（文字列）で検索
   - v2.2: supplier_item_id, warehouse_id（数値）で検索
...
class StockRepository:
    ...
    def find_fifo_lots_for_allocation(
        ...
        v2.2: Updated to use supplier_item_id and warehouse_id (DDL compliant).
        Removed joinedload(Lot.current_stock) - no longer needed.

        Args:
            supplier_item_id: Product ID (not product_code)
        ...
        stmt: Select[tuple[LotReceipt]] = (
            select(LotReceipt)
            .where(LotReceipt.supplier_item_id == supplier_item_id)
