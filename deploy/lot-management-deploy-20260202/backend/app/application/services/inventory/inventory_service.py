...
class InventoryService:
    ...
    def get_inventory_items(
        ...
        if supplier_item_id is not None:
            where_clauses.append("v.supplier_item_id = :supplier_item_id")
            params["supplier_item_id"] = supplier_item_id
        ...
        if result:
            ...
            found_supplier_items = [r.supplier_item_id for r in result]
            ...
            alloc_query = """
                ...
                WHERE r.status IN ('active', 'confirmed')
                  AND l.supplier_item_id IN :supplier_item_ids
                  AND l.warehouse_id IN :warehouse_ids
                GROUP BY r.lot_id, l.supplier_item_id, l.warehouse_id, 
                    CASE 
                        WHEN r.status = 'active' THEN 'soft'
                        WHEN r.status = 'confirmed' THEN 'hard'
                        ELSE 'soft'
            ...
            alloc_rows = self.db.execute(
                text(alloc_query),
                {
                    "supplier_item_ids": tuple(set(found_supplier_items)),
            ...
            count_query_details = """
                SELECT
                    ...
                    COUNT(v.receipt_id) as lot_count
                FROM v_lot_receipt_stock v
                WHERE v.supplier_item_id IN :supplier_item_ids
                  AND v.warehouse_id IN :warehouse_ids
                  AND v.remaining_quantity > 0 AND v.status = 'active'
                GROUP BY v.supplier_item_id, v.warehouse_id
            """
            ...
            for row in count_rows_details:
                key = (row.supplier_item_id, row.warehouse_id)
    ...
    def get_inventory_item_by_product_warehouse(
        ...
        query = """
            ...
            LEFT JOIN supplier_items p ON v.supplier_item_id = p.id
            LEFT JOIN warehouses w ON v.warehouse_id = w.id
            WHERE v.supplier_item_id = :supplier_item_id AND v.warehouse_id = :warehouse_id
        """
        ...
        supplier_query = """
            ...
            WHERE l.supplier_item_id = :supplier_item_id
        ...
        row = self.db.execute(
            text(query), {"supplier_item_id": supplier_item_id, "warehouse_id": warehouse_id}
        ).fetchone()

        if not row:
            # If no inventory record, check if product and warehouse exist in master
            master_query = """
                SELECT
                ...
                WHERE p.id = :supplier_item_id 
                  AND w.id = :warehouse_id
                  AND p.valid_to >= '9999-12-31'
                  AND w.valid_to >= '9999-12-31'
            """
            master_row = self.db.execute(
                text(master_query),
                {"supplier_item_id": supplier_item_id, "warehouse_id": warehouse_id},
            ).fetchone()

            if not master_row:
                return None

            # Return zero-quantity response
            return InventoryItemResponse(
                id=0,  # Dummy ID
                supplier_item_id=master_row.supplier_item_id,
                warehouse_id=master_row.warehouse_id,
                total_quantity=Decimal("0"),
                allocated_quantity=Decimal("0"),
                available_quantity=Decimal("0"),
                soft_allocated_quantity=Decimal("0"),
                hard_allocated_quantity=Decimal("0"),
                active_lot_count=0,
                last_updated=None,
                ...
                supplier_code=master_row.supplier_code,
                supplier_name=master_row.supplier_name,
            )
        ...
        alloc_query = """
            ...
            JOIN v_lot_receipt_stock l ON l.receipt_id = r.lot_id
            WHERE r.status IN ('active', 'confirmed')
              AND l.supplier_item_id = :supplier_item_id
              AND l.warehouse_id = :warehouse_id
            GROUP BY 
                CASE 
                    WHEN r.status = 'active' THEN 'soft'
        ...
        """
        alloc_rows = self.db.execute(
            text(alloc_query), {"supplier_item_id": supplier_item_id, "warehouse_id": warehouse_id}
        ).fetchall()
        ...
        count_query = """
            ...
            WHERE supplier_item_id = :pid AND warehouse_id = :wid 
            AND remaining_quantity > 0 AND status = 'active'
        """
        count_res = self.db.execute(
            text(count_query), {"pid": supplier_item_id, "wid": warehouse_id}
        ).scalar()
    ...
    def get_inventory_by_product(self) -> list[dict]:
        ...
        query = """
            ...
            FROM v_lot_receipt_stock l
            WHERE l.remaining_quantity > 0 AND l.status = 'active'
            GROUP BY l.supplier_item_id, l.display_name, l.product_code
    ...
    def get_filter_options(
        ...
        Args:
            supplier_item_id: Currently selected product ID
    ...
    def _get_filter_options_from_master(
        ...
        # Phase1: Include customer item mappings if they exist
        p_query = """
            SELECT 
                si.id, 
                si.maker_part_no, 
            ...
            LEFT JOIN customer_items ci ON si.id = ci.supplier_item_id AND ci.valid_to >= '9999-12-31'
            LEFT JOIN customers c ON ci.customer_id = c.id AND c.valid_to >= '9999-12-31'
            WHERE si.valid_to >= '9999-12-31'
        """
        params: dict[str, Any] = {}
        if supplier_id:
            p_query += " AND si.supplier_id = :supplier_id"
            params["supplier_id"] = supplier_id

        p_rows = self.db.execute(text(p_query), params).all()
        products = []
        for r in p_rows:
            display_code = r.customer_part_no if r.customer_part_no else r.maker_part_no
            display_name = r.display_name
            if r.customer_name:
                display_name = f"{display_name} ({r.customer_name})"

            products.append(InventoryFilterOption(id=r.id, code=display_code, name=display_name))
    ...
    def _get_filter_options_from_stock(
        ...
        if tab != "in_stock":
            ...
            m_where_str = ""
            if m_clauses:
                m_where_str = "AND " + " AND ".join(m_clauses)
            ...
            SELECT
                'product' AS kind,
                si.id AS id,
                COALESCE(ci.customer_part_no, si.maker_part_no) AS code,
                CASE 
                    WHEN c.customer_name IS NOT NULL THEN si.display_name || ' (' || c.customer_name || ')'
                    ELSE si.display_name
                END AS name
            FROM supplier_items si
            LEFT JOIN customer_items ci ON si.id = ci.supplier_item_id AND ci.valid_to >= '9999-12-31'
            LEFT JOIN customers c ON ci.customer_id = c.id AND c.valid_to >= '9999-12-31'
            WHERE si.valid_to >= '9999-12-31'
            {m_where_str}
            GROUP BY si.id, si.maker_part_no, si.display_name, ci.customer_part_no, c.customer_name
            """
        ...
        candidates_query = (
            ...
            SELECT
                'product' AS kind,
                p.id AS id,
                COALESCE(ci.customer_part_no, p.maker_part_no) AS code,
                CASE 
                    WHEN c.customer_name IS NOT NULL THEN p.display_name || ' (' || c.customer_name || ')'
                    ELSE p.display_name
                END AS name
            FROM stock_base sb
            ...
            GROUP BY p.id, p.maker_part_no, p.display_name, ci.customer_part_no, c.customer_name
